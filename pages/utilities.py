...
def button(url, str, icon):
    icon_base = icon[:2]
    return f"""<a class="btn btn-outline-light btn-sm", href="{url}" target="_blank" rel="noopener noreferrer">
...
def load_publication_data(path):
    ...
    for _, data in yaml_data.items():
        ...
        authors = [
            a if a != "me" else "<strong>Maria Grazia Puxeddu</strong>" for a in authors
        ]
